package com.example.demo.cc.saml;

import com.example.demo.cc.xml.PrettyPrinter;
...
public class SAML {
	...
	/**
	 * Parse the command line for a filename to read, and optionally a filename to
	 * write (absent which the application will write to the console). Reads the
	 * given file as an XMLObject, and then dumps using a simple
	 * {@link cc.xml.PrettyPrinter pretty printer}.
	 */
	public static void main(String[] args) throws Exception {
		if (args.length == 0) {
			System.out.println("Usage: java cc.saml.SAML <inputFile> [<outputFile>]");
			System.exit(-1);
		}
		...
		if (command.equals("generate")) {
			...
			if (type.equals("authn"))
				handler.printToFile(
						handler.createAuthnAssertion(handler.createSubject("harold_dt", null, "sender-vouches", null),
								AuthnContext.PPT_AUTHN_CTX),
						null);

			else if (type.equals("attr")) {
				Subject subject = handler.createSubject("louisdraper@abc.gov", NameID.EMAIL, null, null);
				...
				System.out.println("Usage: java cc.saml.SAML <generate> authn|attr");
				System.exit(-1);
			}
		} else {
	...
	/**
	 * Any use of this class assures that OpenSAML is bootstrapped. Also initializes
	 * an ID generator.
	 */
	static {
		try {
	 ...
	 *                           element and use this as the Method attribute; must
	 *                           be "sender-vouches" or "bearer", as HOK would
	 *                           require additional parameters and so is NYI
	 */
	public Subject createSubject(String username, String format, String confirmationMethod, String recipient) {
		...
		if (confirmationMethod != null) {
			...
			SubjectConfirmationData subConData = create(SubjectConfirmationData.class,
	...
	/**
	 * Returns a SAML assertion with generated ID, current timestamp, given subject,
	 * and simple time-based conditions.
	 * 
	...
	/**
	 * Helper method to generate a shell response with a given status code and query
	 * ID.
	 */
	...
	/**
	 * Helper method to generate a shell response with a given status code, status
	 * message, and query ID.
	 */
	 ...
	 * @param authnCtx The "authentication context class reference", e.g.
	 *                 AuthnContext.PPT_AUTHN_CTX
	 */
